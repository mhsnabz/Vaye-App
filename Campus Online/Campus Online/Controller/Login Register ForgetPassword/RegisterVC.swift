            ...
            Auth.auth().createUser(withEmail: userEmail!, password: pass!) { (result, error) in
                ...
                else
                {
                    if result != nil {
                        ...
                        dic = ["number":number!,"name":_name!,"profileImage":"","thumb_image":"","email":result!.user.email!,"priority":"student","uid":result!.user.uid,"short_school":self.school.shortName! ,"schoolName":self.school.name!,"username":username!]
