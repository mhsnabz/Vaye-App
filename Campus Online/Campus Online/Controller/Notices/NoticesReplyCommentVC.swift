//
//  NoticesReplyCommentVC.swift
//  Campus Online
//
//  Created by mahsun abuzeyitoğlu on 8.12.2020.
...
class NoticesReplyCommentVC: UIViewController {
    ...
    init(comment : CommentModel , currentUser : CurrentUser , post : NoticesMainModel) {
...
extension NoticesReplyCommentVC : UITableViewDataSource , UITableViewDelegate {
...
extension NoticesReplyCommentVC : CommentDelegate {
    ...
    func likeClik(cell: CommentMsgCell) {
        ...
        NoticesService.shared.setRepliedCommentLike(repliedComment: repliedComment, likedCommentId: comment.commentId!, currentUser: currentUser, post: post)
        {[weak self](_val) in
            guard let sself = self else { return }
            if _val{
                sself.tableView.reloadData()
