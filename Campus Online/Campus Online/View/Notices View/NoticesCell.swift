...
class NoticesCell: UICollectionViewCell {
    weak var delegate : NewPostNoticesVCDelegate?
    weak var noticesPost : NoticesMainModel?{
        didSet{
            configure()
            guard let currentUser = currentUser else { return }
            checkIsDisliked(user: currentUser, post: noticesPost) {[weak self] (_val) in
            ...
            checkIsLiked(user: currentUser, post: noticesPost) {[weak self] (_val) in
                   guard let s = self else { return }
                if _val{
